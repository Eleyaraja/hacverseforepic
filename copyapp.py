...
# Modified preprocessing function with validation
def preprocess_image(image):
    try:
        # Convert to RGB and validate channels
        if image is None or image.size == 0:
            st.error("Empty image received")
            return None
        ...
        if np.std(image) < 5:  # Detect low variance images
            st.error("Invalid image: Low contrast detected")
            return None

        # CLAHE with retina-specific parameters
        lab = cv2.cvtColor(image, cv2.COLOR_RGB2LAB)
        l_channel = lab[:,:,0]
...
def generate_retinal_heatmap(model, original_image):
    try:
        # Input validation
        if original_image is None or original_image.size == 0:
            st.error("Invalid input image")
            return None

        # Preprocess with enhanced checks
        processed = preprocess_image(original_image.copy())
        if processed is None or processed.shape != (IMG_SIZE, IMG_SIZE, 3):
            st.error("Invalid preprocessing output")
            return None

        # Model input preparation
        img_array = np.expand_dims(processed, axis=0)
        if img_array.min() < 0 or img_array.max() > 1.0:
            st.error("Invalid pixel range")
            return None
        ...
        return cv2.addWeighted(
            cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB), 0.7,
            (heatmap_colored * 255).astype(np.uint8), 0.3, 0
        )
...
# Modified Grad-CAM function for retinal images
@st.cache_resource
...
def main():
    ...
    if page == "📋 Home":
        ...
        with tab2:
            if diagnosis != "Healthy":
                st.subheader("Clinical Attention Map")
                ...
                try:
                    ...
                    original_img = st.session_state.image.copy()
                    ...
                    heatmap_img = generate_retinal_heatmap(
                    ...
                    if heatmap_img is not None:
                        ...
                        with col2:
                            st.image(
                                heatmap_img,
                                caption=f"Pathology Focus: {diagnosis}",
                                use_column_width=True
                            )
                        
                        st.markdown("""
                        **Clinical Interpretation Guide:**
                        - 🔴 Red areas show regions influencing diagnosis
                        - Compare with standard retinal landmarks
                        - Consider vascular patterns and lesion locations
                        """)
